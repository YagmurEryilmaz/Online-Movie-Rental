...
public interface FriendRequestRepository extends CrudRepository<FriendRequest,Long> {
    ...
    @Query("select case " +
            "WHEN fr.primaryKey.sender_id = :userId THEN fr.receiver.email\n" +
            "WHEN fr.primaryKey.receiver_id = :userId THEN fr.sender.email " +
            "ELSE '' END  from FriendRequest fr where fr.friendReq_status='accepted' and (fr.primaryKey.sender_id = :userId or fr.primaryKey.receiver_id = :userId)")
    List<String> getFriendsEmail(@Param("userId") Long userId);
